...
class CheckPaymentStatus extends Command
{
    ...
    protected $description = 'Récupère le statut d\'un paiement depuis NOWPayments et met à jour la base de données';
    ...
    public function handle()
    {
        ...
        foreach ($payments as $payment) {
            // 2. On détermine quel identifiant envoyer au service NOWPayments
            // On priorité le provider_id (ex: 5006250540), sinon la référence
    ...
    {
        $transaction = Payment::where('provider_id', $paymentId)->first();
        ...
        if ($data['payment_status'] === 'finished' && $transaction->status !== 'success') {
            $transaction->update([
                'status' => 'success',
                'tx_hash' => $data['payin_hash'] ?? $transaction->tx_hash,
                'processed_at' => now(),
            ]);
